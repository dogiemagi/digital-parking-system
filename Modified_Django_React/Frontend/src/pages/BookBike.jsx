...
function BookBike() {
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      try {
        const data = await getSlots("Bike"); // POST /api/slots handled in api.js
  ...
  const validate = () => {
    ...
    const minimumBookingTime = 60 * 60 * 1000; // 1 hour
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      vehicle_number: vehicleNumber,   // match Flask backend keys
      vehicle_owner: vehicleOwner,
      slot_number: parseInt(slotNumber),
      in_time: inTime,
      out_time: outTime,
      payment_method: paymentMethod,
      rate_per_hour: 2, // Bike rate
    };

    try {
      const data = await bookSlot(payload); // POST /api/book handled in api.js
      if (data.success) {
  ...
  return (
    <Layout>
      <div className="container">
        <h2 style={{ color: "white" }}>Book a Parking Slot for Bike</h2>
